...
# เพิ่มเฉพาะการ push แบบแบ่งชุด
BATCH_PUSH_FILE_COUNT = 20
BATCH_PUSH_SLEEP_SEC = 3
...
def parse_links_file(link_path):
    ...
    for line in lines:
        if "http" in line:
            urls.append(line)
            episodes.append({
                "ep_title": current_title if current_title else "Untitled Episode",
                "ep_url": line
            })
        else:
            current_title = line
...
def shell_quote_path(path_text):
    path_text = str(path_text).replace('"', '\\"')
    return f'"{path_text}"'

def git_get_changed_files(repo_path):
    code, out, err = run_git("git status --porcelain", repo_path, timeout=120)
    if code != 0:
        return None, err or out
...
def git_stage_files(repo_path, files):
    if not files:
        return True, ""

    quoted = " ".join(shell_quote_path(f) for f in files)
    code, out, err = run_git(f"git add -- {quoted}", repo_path, timeout=180)
    if code != 0:
        return False, err or out

    return True, ""

def git_stage_deletions(repo_path):
    code, out, err = run_git("git add -u", repo_path, timeout=180)
    if code != 0:
        return False, err or out
    return True, ""

def git_commit_current_index(repo_path, message):
    code, out, err = run_git(f'git commit -m "{message}"', repo_path, timeout=180)
    if code != 0:
...
def git_commit_in_batches(repo_path, message, batch_size=20, sleep_sec=3, allow_force_push=False):
    changed, err = git_has_changes(repo_path)
    if err:
        return False, err, 0
    ...
    changed_files, err = git_get_changed_files(repo_path)
    if err:
        return False, err, 0

    if changed_files is None:
        return False, "cannot get changed files", 0

    committed_count = 0

    ok, err = git_stage_deletions(repo_path)
    if not ok:
        return False, err, committed_count

    for batch_no, batch_files in enumerate(chunk_list(changed_files, batch_size), start=1):
        ok, err = git_stage_files(repo_path, batch_files)
        if not ok:
            return False, err, committed_count

        ok, err = git_commit_current_index(repo_path, f"{message} (batch {batch_no})")
        if not ok:
            return False, err, committed_count
        ...
        remaining_changed, remaining_err = git_has_changes(repo_path)
        if remaining_err:
            return False, remaining_err, committed_count

        if remaining_changed:
            time.sleep(sleep_sec)

    return True, "", committed_count

def git_commit_if_needed(repo_path, message):
    ok, err, committed_count = git_commit_in_batches(
        repo_path=repo_path,
        message=message,
        batch_size=BATCH_PUSH_FILE_COUNT,
        sleep_sec=BATCH_PUSH_SLEEP_SEC,
...
# STEP 2
# ==========================================
def build_file_url(category, full_path, cat_path):
    ...
    for cat in sorted(os.listdir(LIBRARY_ROOT)):
        ...
        for root, dirs, files in os.walk(cat_path):
            ...
            for f in sorted(files):
                ...
                save_state(2, "build_item", cat, root, f, "running")
                ...
                item_data = {
                    ...
                    "url": build_file_url(cat, full_p, cat_path),
                    "category": cat,
    ...
    if is_git_repo(DB_DIR):
        ...
        if PUSH_DB_ALWAYS:
            ...
            if not ok:
                ...
                print("⏩ ข้าม DB_DIR (ไม่มีการเปลี่ยนแปลง)")
            else:
