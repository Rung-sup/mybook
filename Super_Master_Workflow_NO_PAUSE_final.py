...
def git_commit_if_needed(repo_path, message):
    ...
    code, out, err = run_git("git add .", repo_path, timeout=120)
    if code != 0:
        return False, err or out, False

    code, out, err = run_git(f'git commit -m "{message}"', repo_path, timeout=120)
    if code != 0:
...
def git_commit_and_push_in_batches(repo_path, base_message, allow_force=False, batch_size=PUSH_BATCH_SIZE, delay_sec=PUSH_BATCH_DELAY_SEC):
    ...
    for batch_index, batch in enumerate(chunked(changed_files, batch_size), start=1):
        ...
        code, out, err = run_git(f'git commit -m "{commit_message}"', repo_path, timeout=180)
        if code != 0:
            text = (err or out).lower()
            if "nothing to commit" in text:
                continue
        ...
        for root, dirs, files in os.walk(cat_path):
            ...
            for f in sorted(files):
                ...
                item_data = {
                    ...
                    "url": build_file_url(repo_category, full_p, cat_path),
                    "category": display_category,
                    "source_repo": repo_category,
...
def sync_single_repo(repo_path, commit_message, allow_force_push=False):
    ...
    ok, msg = git_commit_and_push_in_batches(
        repo_path=repo_path,
        base_message=commit_message,
        allow_force=allow_force_push,
        batch_size=PUSH_BATCH_SIZE,
        delay_sec=PUSH_BATCH_DELAY_SEC
    )
    return ok, msg
...
def step3_sync(state):
    ...
    if is_git_repo(DB_DIR):
        ...
        if PUSH_DB_ALWAYS:
            ok, msg = git_commit_and_push_in_batches(
                repo_path=DB_DIR,
                base_message="Update Audiobook DB",
                allow_force=False,
                batch_size=PUSH_BATCH_SIZE,
                delay_sec=PUSH_BATCH_DELAY_SEC
            )
