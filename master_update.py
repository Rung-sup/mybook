...
def main():
    ...
    for cat in categories:
        ...
        if cat.startswith("7_"):
            print(f"🎵 กำลังสแกนหมวดเพลง: {cat}")
            for root, dirs, files in os.walk(cat_path):
                # ตรวจสอบว่าในโฟลเดอร์นี้มีไฟล์ทั้งหมดกี่ไฟล์
                for file_name in files:
                    ext = file_name.lower().split('.')[-1]
                    # เพิ่มนามสกุลที่รองรับให้มากขึ้น
                    if file_name.lower().endswith(('.mp3', '.m4a', '.flac', '.wav', '.aac', '.wma')):
                        full_path = os.path.join(root, file_name)
                        f_size_mb = os.path.getsize(full_path) / (1024 * 1024)
                        
                        # พิมพ์ชื่อไฟล์ที่เจอออกมาเลยเพื่อความมั่นใจ
                        print(f"   🔎 ตรวจพบเพลง: {file_name} ({f_size_mb:.2f} MB)")
